	...
	if ( ! current_user_can( 'edit_post', $post_id ) ) return;
        ...
        foreach ( (array) $attachments as $attachments_id => $attachment_value ) {
	        ...
	        if ( sanitize_text_field(  isset($_POST['button_url_buttonurl'.$attachment_value->ID.'']) ) )
	      		update_post_meta( $post_id, 'button_url_buttonurl'.$attachment_value->ID.'', esc_attr( $_POST['button_url_buttonurl'.$attachment_value->ID.''] ) );

	        if ( sanitize_text_field( isset($_POST['button_url_buttontitle'.$attachment_value->ID.''] ) ) )
	      		update_post_meta( $post_id, 'button_url_buttontitle'.$attachment_value->ID.'', esc_attr( $_POST['button_url_buttontitle'.$attachment_value->ID.''] ) );

	        if ( sanitize_text_field( isset($_POST['imgtitle'.$attachment_value->ID.'']) ) )
	      		update_post_meta( $post_id, 'imgtitle'.$attachment_value->ID.'', esc_attr($_POST['imgtitle'.$attachment_value->ID.'']) );

	        if ( sanitize_text_field(isset($_POST['imgdescription'.$attachment_value->ID.'']) ) )
	      		update_post_meta( $post_id, 'imgdescription'.$attachment_value->ID.'', esc_attr( $_POST['imgdescription'.$attachment_value->ID.''] ) );
	      }
