...
Class ThreeD_Image_Slider
{
  ...
  static function ThreeDImageSlidet_Html_container( $attachments )
  {
    ...
    echo '<div class="spartan-gallery-wrapper w-gallery">';
      echo '<section id="responsiveGallery-container" class="responsiveGallery-container">';
        echo '<a class="responsiveGallery-btn responsiveGallery-btn_prev" href="javascript:void(0);"></a>';
        echo '<a class="responsiveGallery-btn responsiveGallery-btn_next" href="javascript:void(0);"></a>';
          echo '<ul class="responsiveGallery-wrapper">';
            foreach ( $attachments as $attachments_id => $attachment_val ) {
              ...
              echo '<li href="'.$attachment_val->guid.'"  class="js-img-viwer img-true-location responsiveGallery-item" data-caption="" data-id="'.$attachments_id.'">';

                if ( $image_options['display_original'] == 'on' and $image_options['thumbnail_options'] == 'off' ) {
                  echo '<a href="#" class="responsivGallery-link">';
                    echo '<img style="height:'.$image_options['image_height'].''.$image_options['image_measure_value'].';  width:'.$image_options['image_width'].''.$image_options['image_measure_value'].';" src="'.$attachment_val->guid.'"  />';
                  echo '</a>';
                } elseif ( $image_options['thumbnail_options'] == 'on' and $image_options['display_original'] == 'off' ) {
                  echo '<a href="#" class="responsivGallery-link">';
                    echo '<img style="height:'.$image_options['image_height'].''.$image_options['image_measure_value'].';  width:'.$image_options['image_width'].''.$image_options['image_measure_value'].';" src="'.$image_attributes[0].'"  />';
                  echo '</a>';
                } else
                {
                ...
                echo '<div style="color: black;" class="w-responsivGallery-info">';
                  if( ! empty( implode(get_post_meta( get_the_ID(),"imgtitle" . $attachment_val->ID))) ) {
                    echo '<h4 class="text spartan-gallery-img-title" >'.esc_html(implode(get_post_meta( get_the_ID(),"imgtitle" . $attachment_val->ID) ) ).'</h4>';
                  }
                  if( ! empty( implode(get_post_meta( get_the_ID(),"imgdescription" . $attachment_val->ID))))  {
                    echo '<p class="text spartan-gallery-img-description" >'.esc_html(implode(get_post_meta( get_the_ID(),"imgdescription" . $attachment_val->ID) ) ).'</p>';
                  }
